...
int main(int argc, char **argv)
{
    ...
    if( cmd == CMD_LINKM_BOOTLOADGO ) {
        ...
        for( int i=0; i < 7; i++ ) {
            ...
            err = linkm_command(dev, LINKM_CMD_I2CSCAN, 2, 17, cmdbuf, recvbuf);
            if( err ) {
        ...
        printf("parsing upload script %s\n", file);
        scriptline_t scriptlines[49];

        int n = parse_scriptlines(file, scriptlines);
        printf("parsed %d lines\n", n);
        for( int i=0; i<n; i++ ) {
            scriptline_t s = scriptlines[i];
            printf("writing scriptline %d: %02x %02x %02x %02x\n",
                   s.duration, s.cmd, s.arg0,s.arg1,s.arg2);
            cmdbuf[0] = addr;
            cmdbuf[1] = 'W';  // write script line
            cmdbuf[2] = 0;    // script line to write (only 0 possible)
            cmdbuf[3] = i;    // script line position
            cmdbuf[4] = s.duration;
            cmdbuf[5] = s.cmd;
            cmdbuf[6] = s.arg0;
            cmdbuf[7] = s.arg1;
            ...
            err = linkm_command(dev, LINKM_CMD_I2CTRANS, 9,0, cmdbuf,recvbuf );
            if( err ) {
                fprintf(stderr,"error on upload cmd: %s\n", linkm_error_msg(err));
            }
            linkm_sleep(50);
        }
        /* cmdbuf[0] = addr; */
        /* cmdbuf[1] = 'W'; */
        /* cmdbuf[2] = 0; */
        /* cmdbuf[3] = 1; // script line pos */
        /* cmdbuf[4] = 23; // delay time */
        /* cmdbuf[5] = 'c'; // cmd */
        /* cmdbuf[6] = 0x44; // R */
        /* cmdbuf[7] = 0x55; // G */
        /* cmdbuf[8] = 0x66; // B */
